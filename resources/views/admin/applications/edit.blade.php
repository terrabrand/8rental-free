    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.applications.update", [$application->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tenant') ? 'is-invalid' : '' }}" name="tenant_id" id="tenant_id">
                    @foreach($tenants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tenant_id') ? old('tenant_id') : $application->tenant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_intended_start') ? 'is-invalid' : '' }}" type="text" name="date_of_intended_start" id="date_of_intended_start" value="{{ old('date_of_intended_start', $application->date_of_intended_start) }}">
