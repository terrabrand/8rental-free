...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.landlords.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="image">{{ trans('cruds.landlord.fields.image') }}</label>
                <div class="needsclick dropzone {{ $errors->has('image') ? 'is-invalid' : '' }}" id="image-dropzone">
                </div>
                @if($errors->has('image'))
                    <span class="text-danger">{{ $errors->first('image') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.landlord.fields.image_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="email">{{ trans('cruds.landlord.fields.email') }}</label>
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', '') }}">
                @if($errors->has('email'))
                    <span class="text-danger">{{ $errors->first('email') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.landlord.fields.email_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.landlords.storeMedia') }}',
    maxFilesize: 1, // MB
    ...
    init: function () {
@if(isset($landlord) && $landlord->image)
      var file = {!! json_encode($landlord->image) !!}
